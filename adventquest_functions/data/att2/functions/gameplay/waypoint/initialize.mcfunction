#########################################################################
#Made by Adventquest													#
#Use function to process the Waypoint initialize						#
#########################################################################

scoreboard objectives add WAYPOINT dummy

scoreboard players set count WAYPOINT 0
scoreboard players set timer WAYPOINT 0
scoreboard players set Asunark WAYPOINT 0
scoreboard players set Asunark_launch WAYPOINT 0
scoreboard players set Eolorion WAYPOINT 0
scoreboard players set Eolorion_launch WAYPOINT 0
scoreboard players set Kortaek WAYPOINT 0
scoreboard players set Kortaek_launch WAYPOINT 0
scoreboard players set Meleim WAYPOINT 0
scoreboard players set Meleim_launch WAYPOINT 0
scoreboard players set Owsastr WAYPOINT 0
scoreboard players set Owsastr_launch WAYPOINT 0
scoreboard players set Ryliath WAYPOINT 0
scoreboard players set Ryliath_launch WAYPOINT 0
scoreboard players set Soquai WAYPOINT 0
scoreboard players set Soquai_launch WAYPOINT 0