#################################################################
#Made by Adventquest											#
#Use function to process the green answer 						#
#################################################################


#FRENCH LANGUAGE

tellraw @s[scores={LANGUAGE=0}] {"text":" °-° Hermona : ","color":"green","extra":[{"text":"Merci, vous nous serez d'une grande aide. Il devrait y avoir des bandages encore dans l'es ruine d'Angor. Je possédais en mon ancienne deumeure, un stoque conséquent de bandages. Enfin, s'il n'a pas été consummé par le feu... Ma maison se trouve dans le quartier surplombant la ville. Je vous prierai de me rapporter au minimum 6 paquets de 64 bandages. Bonne chance...","color":"dark_aqua"}]}


#ENGLISH LANGUAGE

tellraw @s[scores={LANGUAGE=1}] {"text":" °-° Hermona : ","color":"green","extra":[{"text":"Thank you, you will be of great help to us. There should still be bandages in the ruins of Angor. I had, in my old residence, a substantial stock of bandages. However, if they have not been consumed by fire ... My house is in the neighborhood overlooking the city. Please bring me a minimum of 6 packs of 64 bandages. Good luck...","color":"dark_aqua"}]}