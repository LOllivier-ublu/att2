...
tellraw @a[scores={LANGUAGE=2},x=-4132,y=69,z=-5652,distance=..7] {"selector":"@a[scores={NUMEROJOUEUR=1}]","extra":[{"text":" : "},{"text":"[ -->]","color":"gray","clickEvent":{"action":"run_command","value":"/execute if score summon2 SQ29 matches 1 run function att2:cinematic/sidequest/29/camp_destroy2"},"hoverEvent":{"action":"show_text","value":"點擊這裡 - 回答 2"}}]}