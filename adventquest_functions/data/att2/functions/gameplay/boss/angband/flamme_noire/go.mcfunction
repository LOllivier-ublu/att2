...
execute if score FlammeNoire ANGOR matches 0..9 store result bossbar minecraft:flamme_noire value run data get entity @e[type=minecraft:ghast,nbt={UUIDMost:91L,UUIDLeast:107L},limit=1] Health
execute if score FlammeNoire ANGOR matches 0..9 store result bossbar minecraft:flamme_noire max run data get entity @e[type=minecraft:ghast,nbt={UUIDMost:91L,UUIDLeast:107L},limit=1] Attributes[0].Base

# Enable song FN Boss if the player die
execute if score FlammeNoire ANGOR matches 0..9 as @a[tag=BattlingFN] run scoreboard players set @s ANGOR_BOSS 0

# Security because Ghast are killed in one Fireball
execute if score FlammeNoire ANGOR matches 0..9 at 00000000-0000-005b-0000-00000000006b as @e[type=minecraft:fireball,tag=FireballSpell,distance=..7] run data merge entity @s {direction:[0.0,4.0,0.0]}
...
# Defeated (victory)
execute if score FlammeNoire ANGOR matches 9 unless entity @e[type=ghast,nbt={UUIDMost:91L,UUIDLeast:107L}] run function att2:gameplay/boss/angband/flamme_noire/start_dying
execute if score FlammeNoire ANGOR matches -2 unless entity @e[type=ghast,nbt={UUIDMost:91L,UUIDLeast:107L}] run function att2:gameplay/boss/angband/flamme_noire/dying
