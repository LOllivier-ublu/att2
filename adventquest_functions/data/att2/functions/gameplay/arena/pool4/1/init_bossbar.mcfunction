#################################################################
#Made by Adventquest											#
#Initialize bossbar health Pool4 Arena1							#
#################################################################

bossbar add minecraft:pool4_a1 {"text":"MERCURIUS","color":"dark_red"}
bossbar set minecraft:pool4_a1 style notched_12
bossbar set minecraft:pool4_a1 players @a
bossbar set minecraft:pool4_a1 color purple
bossbar set minecraft:pool4_a1 name [{"text":"MERCURIUS","color":"red"}]
bossbar set minecraft:pool4_a1 max 20