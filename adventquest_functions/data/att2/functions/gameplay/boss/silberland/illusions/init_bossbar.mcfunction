#################################################################
#Made by Adventquest											#
#Initialize bossbar Illusions									#
#################################################################

bossbar add minecraft:illusion1 {"selector":"00000000-0000-005c-0000-00000000005c","color":"dark_red"}
bossbar set minecraft:illusion1 style notched_12
bossbar set minecraft:illusion1 players @a
bossbar set minecraft:illusion1 color purple
bossbar set minecraft:illusion1 name [{"text":"Illusion","color":"red"}]
execute store result bossbar minecraft:illusion1 max run data get entity 00000000-0000-005c-0000-00000000005c Attributes[0].Base

bossbar add minecraft:illusion2 {"selector":"00000000-0000-006c-0000-00000000006c","color":"dark_red"}
bossbar set minecraft:illusion2 style notched_12
bossbar set minecraft:illusion2 players @a
bossbar set minecraft:illusion2 color purple
bossbar set minecraft:illusion2 name [{"text":"Illusion","color":"red"}]
execute store result bossbar minecraft:illusion2 max run data get entity 00000000-0000-006c-0000-00000000006c Attributes[0].Base

bossbar add minecraft:illusion3 {"selector":"00000000-0000-007c-0000-00000000007c","color":"dark_red"}
bossbar set minecraft:illusion3 style notched_12
bossbar set minecraft:illusion3 players @a
bossbar set minecraft:illusion3 color purple
bossbar set minecraft:illusion3 name [{"text":"Illusion","color":"red"}]
execute store result bossbar minecraft:illusion3 max run data get entity 00000000-0000-007c-0000-00000000007c Attributes[0].Base